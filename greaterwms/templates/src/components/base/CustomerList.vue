  ...
  <q-dialog v-model="formData">
    <q-card class="shadow-24">
      <q-bar class="bg-light-blue-10 text-white rounded-borders" style="height: 50px">
        <div class="text-h6">{{ t(`customer.${mode}`) }}</div>
      </q-bar>
      <q-item-label header>{{ t('customer.name') }}</q-item-label>
      <q-card-section style="max-height: 325px; width: 400px" class="scroll">
        <q-input
          ...
          :rules="[val => val.length > 1 || t('customer.error')]"
        /> </q-card-section>
      <q-item-label header>{{ t('customer.address') }}</q-item-label>
      <q-card-section class="q-pt-none">
        <q-input
          ...
          :rules="[val => val.length > 1 || t('customer.error')]"
        /></q-card-section>
      <q-item-label header>{{ t('customer.manager') }}</q-item-label>
      <q-card-section class="q-pt-none">
        <q-input
          dense
      ...
      <q-item-label header>{{ t('customer.phone') }}</q-item-label>
      <q-card-section class="q-pt-none">
        <q-input
          dense
          v-model="data.phone"
...
const columns = computed(() => [
  { name: 'name', required: true, label: t('customer.name'), align: 'left', field: 'name' },
  { name: 'address', label: t('customer.address'), field: 'address' },
  { name: 'manager', label: t('customer.manager'), field: 'manager' },
  { name: 'phone', label: t('customer.phone'), field: 'phone' },
...
async function submitData(e) {
  await post(`core/customer/${e}/`, data.value)
  .then(() => {
    onRequest()
    cancelSubmit()
  })
