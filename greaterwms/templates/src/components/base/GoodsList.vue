  ...
  <q-dialog v-model="formData">
    <q-card class="shadow-24">
      <q-bar class="bg-light-blue-10 text-white rounded-borders" style="height: 50px">
        <div class="text-h6">{{ t(`goods.${mode}`) }}</div>
      </q-bar>
      <q-item-label header>{{ t('goods.name') }}</q-item-label>
      <q-card-section style="max-height: 325px; width: 400px" class="scroll">
        <q-input
          ...
          :rules="[val => val.length > 1 || t('goods.error')]"
        /> </q-card-section>
      <q-item-label header>{{ t('goods.code') }}</q-item-label>
      <q-card-section class="q-pt-none">
        <q-input
          ...
          :rules="[val => val.length > 1 || t('goods.error')]"
        /></q-card-section>
      <q-item-label header>{{ t('goods.description') }}</q-item-label>
      <q-card-section class="q-pt-none">
        <q-input
          dense
...
const columns = computed(() => [
  { name: 'name', required: true, label: t('goods.name'), align: 'left', field: 'name' },
  { name: 'code', label: t('goods.code'), field: 'code' },
  { name: 'description', label: t('goods.description'), field: 'description' },
...
async function submitData(e) {
  await post(`core/goods/${e}/`, data.value)
  .then(() => {
    onRequest()
    cancelSubmit()
  })
