...
function createData() {
  mode.value = 'create'
  data.value.asn_id = generateUUIDv4()
  data.value.detail = [
        { selected: '', qty: '' },
        { selected: '', qty: '' },
        { selected: '', qty: '' },
...
async function submitData(e) {
  await post(`core/asn/${e}/`, data.value)
  .then(() => {
    onRequest()
    cancelSubmit()
  })
...
watch(() => goodsSearch.value, (val) => {
  ...
  get({
    url: 'core/goods/',
    params: {
      params: JSON.stringify({ data__name__icontains: val, data__code__icontains: val }),
      page: 1,
      max_page: 5
    }
  })
