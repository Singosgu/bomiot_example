        ...
        <q-td :props="props">
          <div v-if="props.col.name === 'action'">
            <q-btn
              ...
              flat
              icon="move_down"
              @click="binMove(props.rowIndex)"
            >
              <q-tooltip
                class="bg-indigo"
                :offset="[10, 10]"
                content-style="font-size: 12px"
              >{{ t('edit') }}库位移动</q-tooltip>
            </q-btn>
          </div>
          <div v-else>
            {{ props.value }}
  ...
  <q-dialog v-model="formData">
    <q-card class="shadow-24">
      <q-bar class="bg-light-blue-10 text-white rounded-borders" style="height: 50px">
        <div class="text-h6">{{ t(`stock.${mode}`) }}</div>
      </q-bar>
...
const columns = computed(() => [
  { name: 'bin_name', required: true, label: t('inventory.bin_name'), align: 'left', field: 'bin_name' },
  { name: 'goods_code', label: t('inventory.goods_code'), field: 'goods_code' },
  ...
  await get({
    ...
    params: {
      params: JSON.stringify({
        data__bin_name__icontains: search.value,
        data__goods_code__icontains: search.value,
        data__goods_name__icontains: search.value,
       }),
...
async function submitData(e) {
  await post(`core/stock/bin/${e}/`, data.value)
  .then(() => {
    onRequest()
    cancelSubmit()
  })
