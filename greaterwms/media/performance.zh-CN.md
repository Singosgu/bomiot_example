...
class PerformanceAPI(HttpUser):
    ...
    def get_fastapi_test(self):
        self.client.get("/fastapi_app/test/", headers=self.headers, name="Get fastapi_app test")
    
    @task(1)
    def get_flask_test(self):
        self.client.get("/flask_app/test/", headers=self.headers, name="Get flask_app test")
```
